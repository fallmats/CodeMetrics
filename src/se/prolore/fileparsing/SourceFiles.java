...
public class SourceFiles {
    ...
    public void parseSrcDir(final File sDir) {
        ...
        for (File file : faFiles) {
            if (file.getName().endsWith("java") && file.isFile()) {
                ...
                SourceFile javaSource = new JavaSourceParser(file.getName(), file.getAbsolutePath());
    ...
    public void addSrcFile(final File sFile) {
        if (sFile.getName().endsWith("java") && sFile.isFile()) {
            SourceFile javaSource = new JavaSourceParser(sFile.getName(), sFile.getAbsolutePath());
            srcList.add(javaSource);
        }
        else {
    ...
    public void countComplexity() {
        for (int i = 0; i < this.getNrOfFiles(); i++) {
            this.getSrcFile(i).getMethods();
            this.getSrcFile(i).countComplexity();
        }
    }

    public void countLines() {
        for (int i = 0; i < this.getNrOfFiles(); i++) {
            this.getSrcFile(i).countLines();
        }
    }
}
