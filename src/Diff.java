...
public class Diff {
    ...
    public Boolean isEqualFiles(String oldFile, String newFile) throws IOException {
        ...
        String md5oldFile = org.apache.commons.codec.digest.DigestUtils.md5Hex(fis);
        fis.close();
        fis = new FileInputStream(new File(newFile));
        String md5newFile = org.apache.commons.codec.digest.DigestUtils.md5Hex(fis);
        fis.close();
        if (md5newFile.equals(md5oldFile)) {
            return true;
        }
        return false;
    }
